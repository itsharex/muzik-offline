...
use crate::components::{
    album::Album, artist::Artist, genre::Genre, hmaptype::HMapType, song::Song,
};
use sled::Tree;
    ...
    match DbManager::new() {
        Ok(dbm) => {
            ...
            let could_insert_albums = {
                let dbm_arced_clone = Arc::clone(&dbm_arced);
                tokio::task::spawn_blocking(move || {
                    insert_map_values_into_respective_tree(
                        &dbm_arced_clone,
                        &albums_hash_map,
                        &String::from("albums"),
                    )
...
pub fn compare_and_set_hash_map(
    hash_map: &mut HashMap<String, HMapType>,
    key: &String,
    value: &HMapType,
) {
    ...
    for song in songs {
        match serde_json::to_string(&song) {
            Ok(song_as_json) => {
                match dbm
                    .song_tree
                    .insert(song.id.to_ne_bytes(), song_as_json.as_bytes())
                {
                    Ok(_) => {}
                    Err(_) => {}
                }
...
pub fn insert_song_into_tree(song: Song) -> Result<(), String> {
    match DbManager::new() {
        Ok(dbm) => match serde_json::to_string(&song) {
            Ok(song_as_json) => {
                match dbm
                    .song_tree
                    .insert(song.id.to_ne_bytes(), song_as_json.as_bytes())
...
fn insert_map_values_into_respective_tree(
    dbm: &DbManager,
    hash_map: &HashMap<String, HMapType>,
    tree_name: &String,
) {
    ...
    for album in albums {
        match serde_json::to_string(&album) {
            Ok(album_as_json) => {
                match dbm
                    .album_tree
                    .insert(album.key.to_ne_bytes(), album_as_json.as_bytes())
                {
                    Ok(_) => {}
                    Err(_) => {}
                }
    ...
    for artist in artists {
        match serde_json::to_string(&artist) {
            Ok(artist_as_json) => {
                match dbm
                    .artist_tree
                    .insert(artist.key.to_ne_bytes(), artist_as_json.as_bytes())
                {
                    Ok(_) => {}
                    Err(_) => {}
                }
    ...
    for genre in genres {
        match serde_json::to_string(&genre) {
            Ok(genre_as_json) => {
                match dbm
                    .genre_tree
                    .insert(genre.key.to_ne_bytes(), genre_as_json.as_bytes())
                {
                    Ok(_) => {}
                    Err(_) => {}
                }
